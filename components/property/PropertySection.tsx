...
import { useCreateListingReviewMutation, useGetListingRatingQuery } from "@/redux/api/houseApi";
...
const PropertySection = () => {
  ...
  const { data: ratingData, isSuccess, isLoading } = useGetListingRatingQuery(propertyData?.id)
  ...
  const [createListingReview, { isLoading: loadingReview, isSuccess: successReview, isError: isErrorReview, error: errorReview }] = useCreateListingReviewMutation()
  const onSubmitReview = async () => {
    ...
    if (reviewRating && reviewComment) {
      await createListingReview({ rating: reviewRating, review: reviewComment, apartment_id: propertyData?.id })
    }
  }
  useEffect(() => {
    if (isSuccess) {
  ...
  }, [data, dispatch, errorReview, isErrorReview, isSuccess, propertyData, ratingData, successReview])

  return (
    <>
      {loading ? <Spinner /> : <div className="flex flex-col-reverse md:grid md:grid-cols-[75%_25%] w-full md:mt-[6rem]">
        ...
        <div className="p-[0.5rem] border border-[#D6DDEB] px-[20px] py-[13px] flex flex-col gap-[0.3rem]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[30%_70%]">
            {isLoading ? <Spinner /> : <div className="flex md:flex-col items-center">
              <h1 className="text-[#000] font-[500] text-[32px]">{String(rating).slice(0, 3)}</h1>
        ...
        <div className="p-[0.5rem] border border-[#D6DDEB] px-[20px] py-[13px] flex flex-col gap-[0.3rem]">
          ...
          <div className="">
            <p>leave a review</p>
            <ReactQuill theme="snow" placeholder="Message" value={reviewComment} onChange={setReviewComment} />
            <div className="flex justify-between items-end">
              ...
              <Button type="primary" disabled={loadingReview} className="!bg-[#515B6F]" onClick={onSubmitReview}>
