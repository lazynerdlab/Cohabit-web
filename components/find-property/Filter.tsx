...
const Filter = ({ setLocations,
  ...
  const generateUpdatedAreaQueryString = () => {
    if (locations) {
      const filteredAreas = Array.isArray(locations)
        ? locations.map((state: Record<string, any>) => state.value || state.id).filter(Boolean)
        : [locations].map((state: Record<string, any>) => state.value || state.id).filter(Boolean);
  ...
  }, [areaIsSuccess, areaData, budgetSuccess, isSuccess, budgetData?.data, data?.data, isError, areaIsError, budgetIsError, error])
  // const [areas, setAreas] = useState<Tag[]>([]);

  const items: CollapseProps['items'] = [
    {
      key: '1',
      label: <span className="font-medium text-colorPrimary">Property Type</span>,
      children: <div className="max-h-[300px] overflow-auto">
        <FilterTags options={propertyOptions}
    ...
    },
    {
      ...
      children: <>
        <FilterTags options={locationOptions}
    ...
    },
    {
      ...
      children: <div className="max-h-[300px] overflow-auto">
        <FilterTags options={areasOptions}
    ...
    },
    {
      ...
      children: <div className="max-h-[300px] overflow-auto">
        <FilterTags options={budgetOptions}
